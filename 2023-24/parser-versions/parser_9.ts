...
const clamp = (path: string, value: number, min = 1, max = 0xffff) =>
  value < min
    ? no([path, "range"], `Min ${min}`, value)
    : value > max
    ? no([path, "range"], `Max ${max}`, value)
    : value;
const OPTION = /^[^\n\t,]{1,255}$/, ESCAPE = /[$(-+./?[-^{|}]/g;
const escape = (path: string, options: string[]) => {
  let a = "", z = 0, Z = clamp(path, options.length);
  do a += "|" + test(path, OPTION, options[z]).replace(ESCAPE, "\\$&"); while (
    ++z < Z
  );
  return a.slice(1);
};
...
export class Parser {
  ...
  constructor(public parts: Part[]) {
    ...
    const c = Array<RegExp>(clamp("parser > parts", parts.length, 1, 0xff) + 1);
    let d = "^(?=.{1,65535}$)([0-9A-Za-z]{45})", e = 0;
    let f, g, h, i, j, z = 0, y;
    do switch (
      (b(test("parser > key", KEY, (f = parts[z])[0])), a.push(f[1]), f)[1]
    ) {
      case 0:
        g = "(?:yes|no)", ++e;
        break;
      case 1:
        i = clamp("parser > type 1", f[3], 0, 255);
        y = clamp("parser > type 1", f[2], 0, i);
        a.push(y, i), g = "(?:";
        do g += y + "|"; while (++y <= i);
        g = g.slice(0, -1) + ")", ++e;
        break;
      ...
      case 3:
        ...
        break;
      case 4:
        g = `(?:${escape("parser > type 4", h = f[2])})`, e += 2, y = 0;
        do b(h[y]); while (++y < h.length);
        a.push(0);
        break;
      case 5:
        g = `(?:(?:${
          escape("parser > type 5", h = f[2])
        })(?:, |(?=\\t|$))){0,${h.length}}`;
        e += 4, y = 0;
        do b(h[y]); while (++y < h.length);
        a.push(0);
      ...
      case 6:
        g = `[^\\n\\t]{${i = f[2]},${j = f[3]}}`;
        clamp("parser > type 6", i, 0, clamp("parser > type 6", j));
        e += j + 2, a.push(i, i >> 8, j, j >> 8);
        break;
      case 7:
        g = `(?:\\d{1,10}(?:, |(?=\\t|$))){${i = f[2]},${j = f[3]}}`;
        clamp("parser > type 7", i, 0, clamp("parser > type 7", j, 1, 0x3fff));
        e += (j << 2) + 2, a.push(i, i >> 8, j, j >> 8);
        break;
      case 8:
        g = `(?:-?\\d+(?:\\.\\d+)?(?:, |(?=\\t|$))){${i = f[2]},${j = f[3]}}`;
        clamp("parser > type 8", i, 0, clamp("parser > type 8", j, 1, 0x1fff));
        e += (j << 3) + 2, a.push(i, i >> 8, j, j >> 8);
        break;
      case 9:
        g = `(?:(?:${
          escape("parser > type 9", h = f[4])
        })(?:, |(?=\\t|$))){${i = f[2]},${j = f[3]}}`;
        clamp("parser > type 9", i, 0, clamp("parser > type 9", j, 1, 0x7fff));
        e += (j << 1) + 2, a.push(i, i >> 8, j, j >> 8), y = 0;
        do b(h[y]); while (++y < h.length);
        a.push(0);
    } while (d += `\\t(${g})`, c[++z] = RegExp(`^${g}$`), z < parts.length);
    this.id = fnv1a(this.raw = new Uint8Array(a));
    (this.patterns = c)[0] = /^[0-9A-Za-z]{45}$/;
  ...
  random(count = 1) {
    ...
    for (let z = 0, y, x, w, d, e, f, g; z < count; ++z) {
      d = u_b62(crypto.getRandomValues(new Uint8Array(32)));
      crypto.getRandomValues(b), y = x = 0;
      do switch ((d += "\t", e = this.parts[y])[1]) {
        ...
        case 1:
          ...
          break;
        case 2:
          d += c.getUint32(x, true), x += 4;
          break;
        case 3:
          d += +(c.getUint32(x, true) * Math.random()).toFixed(1), x += 8;
          break;
        case 4:
          d += e[2][c.getUint16(x, true) % e[2].length], x += 2;
          break;
        case 5:
          w = (f = e[2]).length, g = c.getUint32(x, true), x += 4;
          do if (g & 1 << --w) d += f[w] + ", "; while (w);
          break;
        case 6:
          w = 0, f = between(e[2], e[3]);
        ...
        case 7:
          w = 0, f = between(e[2], e[3]);
          do d += c.getUint32(x, true) + ", "; while (x += 4, ++w < f);
          break;
        case 8:
          w = 0, f = between(e[2], e[3]);
          do d += +(c.getUint32(x, true) * Math.random()).toFixed(1)
            + ", "; while (x += 8, ++w < f);
          break;
        case 9:
          w = 0, f = e[4], g = between(e[2], e[3]);
          do d += f[c.getUint16(x, true) % f.length] + ", "; while (
            x += 2, ++w < g
          );
  ...
  parse(path: string, seeds: U32, value: string) {
    const a = this.regex.exec(value) ?? no([path, "parse"], this.regex, value);
    const b = b62_u(a[1]);
    const c: Data = { __proto__: null, $$: b, $: get(seeds, b) };
